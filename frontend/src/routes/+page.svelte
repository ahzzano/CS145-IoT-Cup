<script>
  ...
  function handleFileChange(event) {
    ...
    if (!file.type.startsWith('image/')) {
      status = 'error';
      errorMessage = 'Please upload an image file containing your National ID QR code.';
      input.value = '';
      return;
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      status = 'error';
      errorMessage = 'File is too large. Please upload an image under 5MB.';
      input.value = '';
      return;
    }
  ...
  async function handleSubmit(event) {
    ...
    if (!selectedFile) {
      status = 'error';
      errorMessage = 'Please select your National ID QR image before submitting.';
      return;
    }
...
<main class="mx-auto min-h-screen max-w-xl px-4 py-10 sm:px-6">
  <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">VeriTest Enrollment Portal</h1>
    <p class="mt-2 text-sm text-slate-600">
      Upload the QR image from your National ID to verify your eligibility for exam day access.
    </p>
    <p class="mt-4 rounded-md bg-blue-50 px-3 py-2 text-xs text-blue-800">
      Your National ID image is used solely for verification and securely stored.
    </p>

    <form class="mt-6 space-y-4" onsubmit={handleSubmit}>
      <div>
        <label for="national-id-qr" class="mb-2 block text-sm font-medium text-slate-800">
        ...
        <input
          ...
          required
        />
        <p class="mt-1 text-xs text-slate-500">Accepted formats: image files up to 5MB.</p>
      </div>
    ...
    {#if status === 'success'}
      <div class="mt-4 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800" role="status" aria-live="polite">
        {statusMessage}
      </div>
    {/if}

    {#if status === 'error'}
      <div class="mt-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800" role="alert" aria-live="assertive">
        {statusMessage}
      </div>
    {/if}
  </section>
